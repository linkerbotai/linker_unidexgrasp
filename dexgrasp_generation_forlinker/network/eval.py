...
from data.dataset import get_mesh_dataloader, get_mesh_dataloader_mypc
...
def main(cfg):
    ...
    """ DataLoaders """
    test_loader = get_mesh_dataloader_mypc(cfg, "test")
    print(len(test_loader))

    """ Trainer """
    trainers = []
    for key in cfg['models'].keys():#rotation、pose
        net_cfg = compose(f"{cfg['models'][key]['type']}_config")#ipdf_config, glow_congif
    ...
    contact_cfg = compose(f"{cfg['tta']['contact_net']['type']}_config")#cm_net_config
    ...
    for key, trainer in zip(cfg['models'].keys(), trainers):
        loader = result_to_loader(result, cfg) if result else test_loader#在第二个网络时会根据第一个网络的result生成loader
        result = []
    ...
    # tta，test-time adaptation 测试时适应
    loader = result_to_loader(result, cfg, cfg['tta']['batch_size'])#根据前两个的result生成loader
    result = []
    for i, data in enumerate(tqdm(loader)):#使用tqdm显示进度条
        points = data['canon_obj_pc'].cuda()#规范化点云数据
        #组合手的平移和旋转和关节角度数据
        hand_pose = torch.cat([data['canon_translation'], torch.zeros_like(data['canon_translation']), data['hand_qpos']], dim=-1)#相同数据类型和形状的全零张量
        old_hand_pose = hand_pose.clone()
        data['hand_pose'] = add_rotation_to_hand_pose(old_hand_pose, data['sampled_rotation'])#添加采样的旋转到手姿势
        plane_parameters = data['canon_plane'].cuda()#平面参数
    ...
    # 保存结果为 .pt 文件
    output_path = os.path.join(args.exp_dir, 'result.pt')
    torch.save(result, output_path)

    result = flatten_result(result)

    # data_root_path_mesh = os.path.join(cfg['dataset']['root_path'], 'DFCData', 'meshes')
    data_root_path_mesh = cfg['dataset']['root_path']

    hand_model = tta_loss.hand_model
    object_model = KaolinModel(
        data_root_path=data_root_path_mesh,
        batch_size_each=1,
        device=cfg['device']
    )

    final_results = []
    for i in trange(len(result['hand_pose'])):#2000
...
def divide(data):
    ...
    for splits_file_name in os.listdir('/media/moning/Newsmy1/from_obj_to_pc/DFCData/splits'):
        with open(os.path.join('/media/moning/Newsmy1/from_obj_to_pc/DFCData/splits', splits_file_name), 'r') as f:
